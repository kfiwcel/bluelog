...
def show_post ( post_id ) :
    ...
    if form.validate_on_submit () :
        ...
        if current_user.is_authenticated :  # send message based on authentication status
            flash ( 'Comment/Reply published.' , 'success' )
        else :
            flash ( 'Thanks, your comment or reply will be published after reviewed.' , 'info' )
...
def reply_comment ( comment_id ) :
    ...
    return redirect (
        url_for ( '.show_post' , post_id=comment.post_id , reply=comment_id ,
                  author=comment.author,comment_body=comment.body) + '#comment-form' )
